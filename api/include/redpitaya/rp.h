...
int rp_GetRuntimeTempAlarm(rp_channel_t channel, bool *status);
  ...
  int rp_CounterGetCounts(double buffer[RP_COUNTER_NUM_COUNTERS]);
  ...
  int rp_CounterCount(double *counts[RP_COUNTER_NUM_COUNTERS], uint32_t numCounts);
  int rp_CounterReset();
  ...
  int rp_CounterWaitAndReadAndStartCounting(double counts[RP_COUNTER_NUM_COUNTERS]);
